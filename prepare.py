...
def imputePerAgeGroup(df, helperDf, string):
    ageValues = pd.unique(df["age"])
    for age in ageValues:
        tempDf = df[(df["age"] == age) & (np.isnan(df[string]))]
        tempHelper = helperDf[helperDf["age"] == age]
        index = tempDf.index
        for l in index:
            df.loc[[l], [string]] = tempHelper[string].median()
    return df


def imputePerAgeGroup(df1, df2, string):
    print("Before1: ", df1[string].isna().sum())
    ...
    print("After1: ", df1[string].isna().sum())
    return df1
...
def commitImputation(df1, df2):
    ...
    print("Before: ", df1['age'].isna().sum())
    df1["age"].fillna(df2["age"].median(), inplace=True)
    print("After: ", df1['age'].isna().sum())
